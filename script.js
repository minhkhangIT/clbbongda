...
async function shareTeams() {
    ...
    // Check if there are results to share
    if (!resultsElement.innerHTML.trim()) {
        alert("Vui lòng chia đội trước khi chia sẻ!");
        return;
    }
    ...
    try {
        ...
        // Convert to blob using a Promise for cleaner async handling
        ...
        const file = new File([blob], 'teams.png', { 
            type: 'image/png',
            lastModified: Date.now() // Help browser recognize file validity
        });

        // Check if Web Share API is actually available and supports files
        ...
        if (canShare && navigator.share) {
            ...
            alert("Trình duyệt không hỗ trợ chia sẻ trực tiếp. Ảnh đã được tải về máy!");
        }
    } catch (err) {
        console.error("Lỗi chia sẻ:", err);
        alert("Không thể thực hiện chia sẻ: " + err.message);
    } finally {
